...
本章给出本文采用的多跳量子中继网络物理模型。该模型以路径为评价对象：一条候选路径由若干量子链路段组成，每一段链路均携带初始保真度、建链时间、噪声类型以及端点相干时间等参数。路由层在扩展候选路径时，根据完整路径段序列重新计算路径时间瓶颈 $T_{\max}$、各段等待时间、存储退相干后的 Bell 对角态以及递归纠缠交换后的端到端保真度。后续章节对路由度量非保序性、单标签 Dijkstra 基线以及切片 Dijkstra 算法的分析，均建立在本章定义的路径级物理语义之上。
...
记为一条路径段。本文以路径段序列描述候选路径，并以各段物理参数作为路径级保真度计算的输入。每一段链路包含以下信息：

\begin{itemize}
  \item 左右端节点名；
  \item 初始保真度 $F_j^{(0)}$；
  \item 噪声模型族 $\mathcal{N}_j$；
  \item 量子链路建立时间 $T_j$；
  \item 左右端节点的相干时间 $\tau$。
\end{itemize}

上述参数由链路层和物理层共同给出。初始保真度 $F_j^{(0)}$ 描述纠缠生成后单跳 Bell 对的初始质量，其数值受信道噪声和纠缠制备过程影响；建链时间 $T_j$ 表示该链路建立纠缠所需的时间；$\tau$ 表示端点量子存储器的相干时间。本文的保真度度量只讨论已具备纠缠生成能力的候选链路，链路成功率可在吞吐量或资源分配模型中另行处理，不作为端到端 Bell 保真度的乘性因子。
...
\end{equation}
其中 $\vert\Phi^\pm\rangle,\vert\Psi^\pm\rangle$ 为四个 Bell 基态。
...
\begin{enumerate}
  ...
  \item 每段链路的就绪时间由其建链时间 $T_j$ 给出；
  \item 整条路径的统一对齐时刻定义为
  \begin{equation}
    T_{\max}(p) = \max_{1 \le j \le h} T_j,
    \qquad T_j \equiv T(e_j);
    \label{eq:path-tmax}
  \end{equation}
...
记路径级递推得到的端到端保真度为 $F_{\mathrm{path}}(p)$。特别地，在\emph{无额外等待}且输入均为 Werner 态的情形，若两条相邻链路初始保真度分别为 $F_1,F_2$，则一次纠缠交换后的 Bell 保真度为
\begin{equation}
  F_{\mathrm{swap}}

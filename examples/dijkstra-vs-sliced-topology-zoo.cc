...
ShortestHopDistances(const Topology& topology, const std::string& src)
{
    std::queue<std::string> pending;
...
std::string
GetXmlAttribute(const std::string& tag, const std::string& name)
{
...
std::string
SanitizeNodeName(const std::string& name)
{
    std::string result;
...
uint32_t
PathHopCount(const std::vector<std::string>& path)
{
    return path.empty() ? 0 : static_cast<uint32_t>(path.size() - 1);
}

GeneratedTopology
LoadTopologyZooGraphml(const std::filesystem::path& graphmlPath,
                       uint32_t seed,
                       uint32_t satelliteLinks,
                       double minSatelliteDistance,
                       double minSatelliteProgress,
                       double maxSatelliteStartX,
                       double maxSatelliteEndX,
                       uint32_t minSatelliteGraphHops,
                       uint32_t maxSatelliteNodeDegree,
                       uint32_t maxPairHops,
                       uint32_t minPairHops,
                       uint32_t pairAttempts)
{
    std::ifstream input(graphmlPath);
    if (!input.is_open())
    {
        NS_FATAL_ERROR("Failed to open GraphML file: " << graphmlPath);
    }

    const std::string xml((std::istreambuf_iterator<char>(input)),
                          std::istreambuf_iterator<char>());
    std::map<std::string, std::string> keyToName;
    const std::regex keyPattern("<key\\b[^>]*>");
    for (std::sregex_iterator it(xml.begin(),
                                 xml.end(),
                                 keyPattern);
         ...
         ++it)
    {
        ...
        const std::string id = GetXmlAttribute(tag, "id");
        const std::string attrName = GetXmlAttribute(tag, "attr.name");
        if (!id.empty() && !attrName.empty())
        {
            keyToName[id] = attrName;
        }
    }
    ...
    std::map<std::string, RawNode> rawNodes;
    std::regex nodePattern("<node\\b([^>]*)>([\\s\\S]*?)</node>");
    for (std::sregex_iterator it(xml.begin(), xml.end(), nodePattern);
         it != std::sregex_iterator();
         ++it)
    {
        const std::string id = GetXmlAttribute((*it)[1].str(), "id");
        if (id.empty())
        {
            continue;
        }
        ...
        RawNode raw;
        raw.name = SanitizeNodeName(id);
        const std::string body = (*it)[2].str();
        const std::regex dataPattern("<data\\b[^>]*key=\"([^\"]*)\"[^>]*>([^<]*)</data>");
        for (std::sregex_iterator dataIt(body.begin(),
                                         body.end(),
                                         dataPattern);
             dataIt != std::sregex_iterator();
             ++dataIt)
        {
            const std::string key = (*dataIt)[1].str();
            const std::string value = (*dataIt)[2].str();
            const std::string attr = keyToName[key];
            if (attr == "Latitude")
            {
                raw.lat = std::stod(value);
                raw.hasGeo = true;
            }
            else if (attr == "Longitude")
            {
                raw.lon = std::stod(value);
                raw.hasGeo = true;
            }
        }
        rawNodes[id] = raw;
    ...
    for (const auto& [id, raw] : rawNodes)
    {
        if (!raw.hasGeo)
        {
            continue;
        }
        minLon = std::min(minLon, raw.lon);
        maxLon = std::max(maxLon, raw.lon);
        minLat = std::min(minLat, raw.lat);
        maxLat = std::max(maxLat, raw.lat);
    }
    const bool hasGeoBounds = minLon < maxLon && minLat < maxLat;

    uint32_t index = 0;
    const uint32_t total = static_cast<uint32_t>(std::max<size_t>(1, rawNodes.size()));
    for (const auto& [id, raw] : rawNodes)
    {
        generated.nodes.push_back(raw.name);
        if (hasGeoBounds && raw.hasGeo)
        {
            generated.positions[raw.name] = {(raw.lon - minLon) / (maxLon - minLon),
                                             (raw.lat - minLat) / (maxLat - minLat)};
        }
        else
        {
            const double angle = 2.0 * M_PI * static_cast<double>(index) / total;
            generated.positions[raw.name] = {0.5 + 0.45 * std::cos(angle),
                                             0.5 + 0.45 * std::sin(angle)};
        }
        ++index;
    }

    std::mt19937 rng(seed);
    std::regex edgePattern("<edge\\b[^>]*source=\"([^\"]*)\"[^>]*target=\"([^\"]*)\"[^>]*>");
    for (std::sregex_iterator it(xml.begin(), xml.end(), edgePattern);
         it != std::sregex_iterator();
         ++it)
    {
        const std::string srcId = (*it)[1].str();
        const std::string dstId = (*it)[2].str();
        if (rawNodes.count(srcId) == 0 || rawNodes.count(dstId) == 0)
        {
            continue;
        }
        const std::string left = rawNodes[srcId].name;
        const std::string right = rawNodes[dstId].name;
        const double distance = Distance(generated.positions[left], generated.positions[right]);
        const LinkMetrics metrics = MakeLocalMetrics(rng, Clamp(distance, 0.0, 1.0));
        AddBidirectionalLink(generated.topology, left, right, metrics);
        AddBidirectionalLink(generated.localTopology, left, right, metrics);
    }

    struct SatelliteCandidate
    {
        std::string left;
        std::string right;
        double score{0.0};
    ...
    std::vector<SatelliteCandidate> candidates;
    for (const auto& left : generated.nodes)
    {
        const auto hopDistance = ShortestHopDistances(generated.localTopology, left);
        for (const auto& right : generated.nodes)
        {
            if (left == right || HasEdge(generated.topology, left, right))
            {
                continue;
            }
            auto hopIt = hopDistance.find(right);
            if (hopIt == hopDistance.end() || hopIt->second < minSatelliteGraphHops)
            {
                continue;
            }
            const Position& lp = generated.positions[left];
            const Position& rp = generated.positions[right];
            if (lp.x > maxSatelliteStartX || rp.x > maxSatelliteEndX)
            {
                continue;
            }
            if (rp.x <= lp.x + minSatelliteProgress)
            {
                continue;
            }
            const double geoDistance = Distance(lp, rp);
            if (geoDistance >= minSatelliteDistance)
            {
                const double jitter = Uniform(rng, 0.0, 0.15);
                const double score = static_cast<double>(hopIt->second) + 2.0 * geoDistance + jitter;
                candidates.push_back({left, right, score});
            }
        }
    }
    ...
    std::sort(candidates.begin(),
              candidates.end(),
              [](const SatelliteCandidate& lhs, const SatelliteCandidate& rhs) {
                  return lhs.score > rhs.score;
              });

    std::map<std::string, uint32_t> satelliteDegree;
    for (const auto& candidate : candidates)
    {
        if (generated.longLinks.size() >= satelliteLinks)
        {
            break;
        }
        if (maxSatelliteNodeDegree > 0 &&
            (satelliteDegree[candidate.left] >= maxSatelliteNodeDegree ||
             satelliteDegree[candidate.right] >= maxSatelliteNodeDegree))
        {
            continue;
        }
        AddLongLink(generated, rng, candidate.left, candidate.right);
        ++satelliteDegree[candidate.left];
        ++satelliteDegree[candidate.right];
    }

    std::vector<std::pair<std::string, std::string>> pairCandidates;
    std::uniform_int_distribution<size_t> nodeDist(0, generated.nodes.size() - 1);
    for (uint32_t attempt = 0; attempt < pairAttempts; ++attempt)
    {
        const std::string& src = generated.nodes[nodeDist(rng)];
        const std::string& dst = generated.nodes[nodeDist(rng)];
        if (src == dst)
        {
            continue;
        }
        const auto path = ShortestPathByHops(generated.localTopology, src, dst);
        const uint32_t hops = PathHopCount(path);
        if (!path.empty() && hops >= minPairHops && hops <= maxPairHops)
        {
            pairCandidates.emplace_back(src, dst);
        }
    }

    if (pairCandidates.empty())
    {
        for (const auto& src : generated.nodes)
        {
            for (const auto& dst : generated.nodes)
            {
                if (src == dst)
                {
                    continue;
                ...
                const auto path = ShortestPathByHops(generated.localTopology, src, dst);
                const uint32_t hops = PathHopCount(path);
                if (!path.empty() && hops <= maxPairHops)
                {
                    pairCandidates.emplace_back(src, dst);
                    break;
                }
    ...
    if (pairCandidates.empty())
    {
        NS_FATAL_ERROR("No routable source/destination pair found in " << graphmlPath);
    }

    std::uniform_int_distribution<size_t> pairDist(0, pairCandidates.size() - 1);
    const auto selected = pairCandidates[pairDist(rng)];
    generated.src = selected.first;
    generated.dst = selected.second;
    std::sort(generated.nodes.begin(), generated.nodes.end());
    return generated;
}
...
RunResult
RunOneTopologyZoo(uint32_t runId,
                  uint32_t seed,
                  const std::filesystem::path& graphmlPath,
                  uint32_t satelliteLinks,
                  double minSatelliteDistance,
                  double minSatelliteProgress,
                  double maxSatelliteStartX,
                  ...
                  uint32_t minSatelliteGraphHops,
                  uint32_t maxSatelliteNodeDegree,
                  uint32_t maxRouteHops,
                  uint32_t minPairHops,
                  uint32_t pairAttempts,
                  uint32_t maxPairRetries,
                  double coherenceMs,
                  double swapSignalDelayMs,
                  uint32_t k,
                  double bucketWidthMs,
                  ...
                  const std::filesystem::path& outputPath)
{
    GeneratedTopology generated = LoadTopologyZooGraphml(graphmlPath,
                                                         seed,
                                                         satelliteLinks,
                                                         minSatelliteDistance,
                                                         minSatelliteProgress,
                                                         maxSatelliteStartX,
                                                         maxSatelliteEndX,
                                                         minSatelliteGraphHops,
                                                         maxSatelliteNodeDegree,
                                                         maxRouteHops,
                                                         minPairHops,
                                                         pairAttempts);

    Ptr<QuantumPhyEntity> qphyent = CreateObject<QuantumPhyEntity>(generated.nodes);
    for (const auto& node : generated.nodes)
    {
        qphyent->SetTimeModel(node, coherenceMs / 1000.0);
    }
    ...
    Ptr<QuantumNetworkLayer> netLayer = CreateObject<QuantumNetworkLayer>();
    netLayer->SetOwner(generated.src);
    netLayer->SetPhyEntity(qphyent);
    ...
    result.dijkstraFidelity = result.dijkstraSuccess ? metric->GetScore(dijkstraLabel) : 0.0;
    result.slicedFidelity = result.slicedSuccess ? metric->GetScore(slicedLabel) : 0.0;
    result.dijkstraTMaxMs = result.dijkstraSuccess ? GetScalar(dijkstraLabel, "t_max_ms") : 0.0;
    result.slicedTMaxMs = result.slicedSuccess ? GetScalar(slicedLabel, "t_max_ms") : 0.0;
    result.dijkstraHopCount = result.dijkstraSuccess ? GetScalar(dijkstraLabel, "hop_count") : 0.0;
    ...
    const bool routeTooLong =
        result.dijkstraHopCount > maxRouteHops || result.slicedHopCount > maxRouteHops;
    if (routeTooLong)
    {
        sliced->Dispose();
        dijkstra->Dispose();
        ...
        if (maxPairRetries > 0)
        {
            return RunOneTopologyZoo(runId,
                                     seed + 1000003,
                                     graphmlPath,
                                     satelliteLinks,
                                     minSatelliteDistance,
        ...
        result.dijkstraSuccess = false;
        result.slicedSuccess = false;
    }
    else if (runActual)
    {
        result.dijkstraActualFidelity =
            result.dijkstraSuccess
                ? SimulateRouteActualFidelity(result.dijkstraRoute,
                                              ...
                                              swapSignalDelayMs)
                : 0.0;
        if (result.slicedSuccess && result.dijkstraSuccess &&
            result.slicedRoute == result.dijkstraRoute)
        {
            result.slicedActualFidelity = result.dijkstraActualFidelity;
        }
        else
    ...
    WriteRunReport(outputPath / ("run_" + runIdText + "_report.md"),
                   generated,
                   result,
                   static_cast<uint32_t>(generated.nodes.size()),
                   0.0,
                   0.0,
                   0,
                   satelliteLinks,
...
main(int argc, char* argv[])
{
    ...
    uint32_t minSatelliteGraphHops = 4;
    uint32_t maxSatelliteNodeDegree = 3;
    ...
    bool runActual = true;
    uint32_t maxRouteHops = 9;
    uint32_t minPairHops = 5;
    uint32_t pairAttempts = 1000;
    uint32_t maxPairRetries = 25;
    std::string graphml = "";
    std::string output = "dijkstra_vs_sliced_topology_zoo_results";

    CommandLine cmd(__FILE__);
    cmd.AddValue("runs", "Number of topology/traffic samples to evaluate.", runs);
    ...
    cmd.AddValue("minSatelliteGraphHops", "Minimum original-graph hop distance for Topology Zoo satellite links.", minSatelliteGraphHops);
    cmd.AddValue("maxSatelliteNodeDegree", "Maximum satellite-overlay degree per node in Topology Zoo mode; 0 disables the cap.", maxSatelliteNodeDegree);
    ...
    cmd.AddValue("runActual", "Run the mixed-state actual-fidelity simulator after routing.", runActual);
    cmd.AddValue("graphml", "Topology Zoo GraphML path. If set, load this real topology.", graphml);
    cmd.AddValue("maxRouteHops", "Skip actual fidelity when either selected route exceeds this hop count.", maxRouteHops);
    cmd.AddValue("minPairHops", "Minimum shortest-hop length for sampled source/destination pairs.", minPairHops);
    cmd.AddValue("pairAttempts", "Random source/destination sampling attempts per run.", pairAttempts);
    cmd.AddValue("maxPairRetries", "Resample source/destination pairs this many times when routed paths exceed maxRouteHops.", maxPairRetries);
    ...
    std::cout << (graphml.empty() ? "Random geometric topology comparison\n"
                                  : "Topology Zoo graphml comparison\n");
    std::cout << "  runs=" << runs
              << " seed=" << seed
              << " nodes=" << nodes
              ...
              << " minSatelliteGraphHops=" << minSatelliteGraphHops
              << " maxSatelliteNodeDegree=" << maxSatelliteNodeDegree
              << " tauMs=" << coherenceMs
